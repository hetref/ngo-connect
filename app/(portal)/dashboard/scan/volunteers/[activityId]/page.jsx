...
const QRScannerPage = () => {
      ...
      if (currentUser?.uid && currentActivityId) {
        try {
          ...
          if (eventDoc.exists()) {
            ...
            const attendedVolunteers = eventData.attendedVolunteers || [];

            if (attendedVolunteers.length > 0) {
              // For each attended volunteer, fetch their name and create a scan entry
              const previousScans = await Promise.all(
                attendedVolunteers.map(async (volunteer) => {
                  const volunteerDoc = await getDoc(
                    doc(db, "users", volunteer.volunteerId)
                  );
                  const volunteerName = volunteerDoc.exists()
                    ? volunteerDoc.data().name || "Unknown"
                    : "Unknown";

                  return {
                    id: volunteer.scannedAt || Date.now(),
  ...
  const updateDatabase = async (parsedData) => {
    try {
      setProcessing(true);
      const { activityId, volunteerId, ngoId, timestamp } = parsedData;

      if (!activityId || !volunteerId) {
        throw new Error("Missing required data in QR code");
      }

      // First check if this volunteer is already marked as attended in the database
      ...
      if (currentUser?.uid) {
        ...
        if (eventDoc.exists()) {
          const eventData = eventDoc.data();
          // Check if this volunteer is already in attendedVolunteers array
          alreadyAttended = eventData.attendedVolunteers?.some(
          ...
          if (alreadyAttended) {
            return {
              success: true,
              volunteerName: "Already Checked In",
      ...
      // Continue with the rest of your existing code for updating the volunteer's record
      ...
      if (volunteerDoc.exists()) {
        ...
        if (currentAttendanceStatus) {
          // Already marked as attended in the volunteer's record
          const volunteerName = volunteerDoc.data().name || "Volunteer";
          return {
            success: true,
            volunteerName,
            alreadyAttended: true,
        ...
        const updatedVolunteeringArray = volunteeringArray.map((volunteer) => {
          if (volunteer.activityId === activityId) {
            return { ...volunteer, attendance: true };
          }
          return volunteer;
        });
      ...
      const activityVolunteerRef = doc(
        db,
        "activities",
        activityId,
        "volunteers",
      ...
      const activityVolunteerDoc = await getDoc(activityVolunteerRef);
      if (
        activityVolunteerDoc.exists() &&
        activityVolunteerDoc.data().attendance
      ) {
        const volunteerName = volunteerDoc.exists()
          ? volunteerDoc.data().name || "Volunteer"
          : "Volunteer";
      ...
      await updateDoc(activityVolunteerRef, {
        attendance: true,
      });
  ...
  const handleScan = async (result) => {
    if (result && !processing) {
      try {
        ...
        // Check if we already scanned this volunteer for this activity
        const isDuplicate = scannedResults.some(
          (scan) =>
            scan.volunteerId === parsedData.volunteerId &&
        ...
        // In handleScan function, after calling updateDatabase:
        if (updateResult.success) {
          if (updateResult.alreadyAttended) {
            showNotification(
              "Already Checked In",
              `${updateResult.volunteerName} was already marked present`,
              "warning"
            );
            ...
            const newScan = {
              ...
              volunteerName: updateResult.volunteerName || "Unknown",
            ...
            showNotification(
              ...
              `${updateResult.volunteerName} has been marked present`,
              "success"
            );

            // Show success animation
            showSuccessAnimation(`${updateResult.volunteerName} checked in!`);
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <span className="text-xs font-mono block mt-1 text-gray-600">
                        ID: {scan.volunteerId?.substring(0, 10)}...
                      </span>
                    </div>
                    <button
